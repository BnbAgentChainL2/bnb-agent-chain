...
(function (root) {
  ...
  function drawAll() {
    ...
    /* 金库页的日聚合图：renderTreasury 时这一页还是 hidden（clientWidth = 0），
       所以真正画它的时机在这里 —— 视图已经可见之后。 */
    var tre = $('#chTre2');
    if (tre) {
      if (VM.daily && VM.daily.treasury && VM.daily.treasury.length) {
        C.draw(tre, { type: 'bar', data: VM.daily.treasury, labels: VM.daily.labels, unit: ' BNB', label: '近 30 日进金库', alt: true });
      } else {
        C.placeholder(tre, VM.st.treasury === 'pre' ? UI.TEXT.PRE : '没有日聚合数据源');
      }
    }
    if (UI.curAgent && $('#chHb')) {
