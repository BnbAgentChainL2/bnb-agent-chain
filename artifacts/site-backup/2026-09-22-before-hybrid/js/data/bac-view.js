...
(function (root) {
  ...
  function statusOf(sec) {
    ...
    if (sec.error) return sec.ready ? 'error' : 'error';
  ...
  function chainStats() {
    ...
    return {
      status: idxStatus(L),
      source: L.source,                         // 'indexer' | 'rpc' | null
      chainId: L.chainId !== null ? L.chainId : C.LAYER_CHAIN_ID,
      head: L.head,
      headTs: L.headTs,
      blockLagSec: L.blockLagSec,
      blockTimeSec: layerSum.blockTimeSec !== undefined ? Number(layerSum.blockTimeSec) : null,
      ...
      degradedNote: BAC.state.indexer.degraded ? TEXT.NO_INDEXER : null
    };
  }
  ...
  function overview() {
    ...
    return {
      ...
      degradedBanner: s.indexer.degraded ? TEXT.NO_INDEXER : null,
