...
(function (root) {
  ...
  function renderAgents() {
    ...
    var s = VM.st.agents;
    if (s !== 'ok') {
      /* 名录在 BSC 的 AgentRegistry 里，代币还没发射、合约还没部署 →
         **没有任何 agent 注册过**。照实说，不编身份。 */
      body.innerHTML = s === 'pre'
        ? R.emptyRow(10, '还没有任何 agent 注册。注册表 AgentRegistry 在 BSC 上，代币还没发射、合约还没部署，'
          + '所以现在一个身份都没有 —— 这不是读取失败，是真的还没有人进场。层内那条链本身已经在出块了，'
          + '区块和交易在「区块」「交易」两页都是实时的。')
        : R.missRow(10, s, '');
  ...
  function renderFeed() {
    ...
    if (s !== 'ok') {
      ul.innerHTML = '<li class="f-none">' + esc(s === 'pre' ? '还没有 agent 动态' : miss(s)) +
        (s === 'pre' ? '　动态要把交易按 agent 归类，这要索引器 + BSC 上的 AgentRegistry，两样都还没上线。'
  ...
  function renderAgentDetail(id) {
    ...
    if (!a) {
      if (s === 'ok' && canAskIdx()) { ask('agent', id); el.innerHTML = R.pageMiss('agent #' + id, 'loading', '正在向索引器要这个身份。'); }
        ...
        ? '还没有任何 agent 注册：AgentRegistry 在 BSC 上，代币还没发射、合约还没部署。'
        : '');
      return;
    }
    UI.curAgent = a;
  ...
  function renderEpochs() {
    ...
    el.innerHTML =
      ...
      '<span class="vh-m">epoch = floor(timestamp / 86400)，由层内块时间推出，<b>当前纪元是实时的</b>；' +
      '锚点要中继把退出根提交到 BSC 的 ChainAnchor，那个合约还没部署，所以下面这张表还是空的</span></div>' +
